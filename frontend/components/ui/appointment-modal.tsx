...
const AppointmentModal: React.FC<AppointmentModalProps> = ({
  ...
  const handleDownloadInvoice = async (appointment_id: string | undefined, token: string | null) => {
    try {
        ...
        >
          ...
          {selectedEvent.status !== "finished" && (
            <div className="flex flex-wrap gap-3 mt-6 justify-end">
              {currentRole === "physiotherapist" && selectedEvent.status === "booked" && isMoreThan48HoursAway() && (
                <button
              ...
              {selectedEvent && new Date(selectedEvent.start) > new Date() && (currentRole === "physiotherapist" || (currentRole === "patient" && isMoreThan48HoursAway())) && (
                <button
