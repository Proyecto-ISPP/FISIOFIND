...
const PatientHistoryViewerModal = ({ patientId, token, onClose }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-lg overflow-hidden">
      {/* Header - Fixed at top */}
      <div className="bg-gradient-to-r from-[#05668D] to-[#6BC9BE] p-4 text-white flex items-center justify-between sticky top-0 z-10">
      ...
      {/* Scrollable Content */}
      <div className="flex-grow overflow-y-auto bg-[rgb(238, 251, 250)] h-[calc(85vh-8rem)]">
        {loading ? (
          <div className="flex items-center justify-center h-32">
            <div className="text-[#05668D] text-lg">Cargando historial clínico...</div>
          </div>
        ) : error ? (
          <div className="bg-red-50 text-red-600 p-4 rounded-lg m-4 text-center">
            {error}
          </div>
      ...
      <div className="bg-white p-3 border-t border-gray-200 flex justify-end sticky bottom-0 z-10">
        <button 
          onClick={onClose}
          className="px-4 py-2 bg-[#05668D] text-white rounded-lg hover:opacity-90 transition-all"
        >
          Cerrar
        </button>
      </div>
...
export default PatientHistoryViewerModal;