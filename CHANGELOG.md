## 🚀 Changelog - 4.0.0

### ✨ Features (44)
- feat: boton asqueroso (@Benjamín Maureira Flores)
- feat: Subtitulos mejorados con mejor visualización (@Ramón Gavira)
- feat: añadido widget de accesibilidad (@Benjamín Maureira Flores)
- feat: setup para reconocimiento de voz (@Ramón Gavira)
- feat: Porject Roadmap actualizado con ppl (@DaniFdezCab)
- feat: despliegue continuo back PPL (@Ramón Gavira)
- feat: workflow PPL (@Benjamín Maureira Flores)
- feat: Añadir nuevos origines para despliegue ppl (@Ramón Gavira)
- Merge pull request #475 from Proyecto-ISPP/feat/ver-documentos (@Antonio Macías)
- Merge pull request #493 from Proyecto-ISPP/feat/update-readme (@Guadalupe Ridruejo Pineda)
- feat: Project Roadmap actualizado (@DaniFdezCab)
- feat: readme actualizado S3 (@DaniFdezCab)
- feat: añadidio estilo de cambio de contraseña a perfil de fisio (@DelfinSR)
- Merge pull request #488 from Proyecto-ISPP/feat/cambiar-claves-digitalocean (@Antonio Macías)
- feat: solucionar el problema de las claves de digital ocean en settings.py (@Francisco Mateos Villarejo)
- feat: terminadas las graficas y cambios de estado (@franciiscocg)
- feat: implementación de valoraciones en el menú de videollamadas (@DanTorBar)
- feat: añadida comprobacion de seguridad para asegurar que un fisio no crea dos ratings (@DelfinSR)
- feat: mandar correo al crear una valoración (@DanTorBar)
- feat: implementar un modal en el perfil de fisioterapeuta si los campos no están en base de datos (@Francisco Mateos Villarejo)
- feat: Consultar saldo, cobrar dinero y generar factura (@franciiscocg)
- feat: frontend de archivos y videos de tratamiento (@Julen Redondo)
- feat: cambiar contraseña en frontend (@DelfinSR)
- feat: actualizacion de contraseña en backend conseguido (@DelfinSR)
- feat: añadidos headers de seguridad a next (@DelfinSR)
- feat: actualizado admin para que implemente la nueva forma de validar dnis unicos (@DelfinSR)
- feat: creada tabla en la bbdd para guardar dnis cifrados para que se pueda comprobar si un dni ya existe (@DelfinSR)
- feat: Hacer que un documento se pueda ver en la aplicación (@Francisco Mateos Villarejo)
- feat: pequeña mejora visual de la lista de valoraciones de un fisioterapeuta (@DanTorBar)
- feat: añadido subida archivos paciente (@Julen Redondo)
- feat: añadido la desuscripcion de correos mediante enlace en email (@danvelcam)
- feat: corrección y mejora de la valoración de citas tras finalizar la videollamada. (@DanTorBar)
- feat: añadido el slider para recibir notificaciones en perfil fisio (@danvelcam)
- feat: contemplado envio de correos segun estado notificaciones (@danvelcam)
- feat: añadida configuracion de recepcion de notificaciones (@danvelcam)
- feat: lista de valoraciones en perfil de fisio (@DanTorBar)
- feat: Modificación del perfil de fisioterapeuta para añadir los nuevos atributos (@Francisco Mateos Villarejo)
- feat: Actualizar el perfil de fisioterapeuta con cuatro nuevos campos (@Francisco Mateos Villarejo)
- feat: sonarqube workflow ahora muestra coverage en la pantalla (@DelfinSR)
- feat: realizados tests para la funcion de obtener y editar el horario semanal de un fisioterapeuta (@MiguelEncina)
- feat: completados tests de actualización y borrado de citas y actualizados algunos errores (@MiguelEncina)
- feat: inclusión de opción de comentar en el modal de la cita (mis-citas) (@DanTorBar)
- feat: valoración dentro del modal de la cita (mis-citas) (@DanTorBar)
- feat: creación de modelo de valoración y modal correspondiente para la valoración de una cita tras finalizarse (@DanTorBar)
### 🐛 Fixes (51)
- fix: correccion ortografica (@Miguel Encina)
- fix: boundary en despliegue (@Benjamín Maureira Flores)
- fix: cambio de ppl a wpl (@Antonio Macías)
- fix: correcciones de ortografía y errores menores (@Antonio Macías)
- fix: correcciones menores (@Miguel Encina)
- fix: video and file management for physio now working (@Antonio Macías)
- fix: arregladas pequeñas imprecisiones (@Guadalupe Ridruejo Pineda)
- fix: vuelta atras, he cambiado el .env (@Benjamín Maureira Flores)
- fix: arreglo en ruta de confirmation email (@Benjamín Maureira Flores)
- Merge pull request #503 from Proyecto-ISPP/hotfix/perfil-paciente (@Guadalupe Ridruejo Pineda)
- fix: se aplasto el cambio (@Benjamín Maureira Flores)
- fix: genero fisio (@guaridpin)
- fix: correcion de imagenes que no se veian en la carpeta del s3 (@Antonio Macías)
- fix: Arreglos sobre estructura de proyecto e indice de proyecto (@DaniFdezCab)
- fix: pequeñas correcciones de formato (@Antonio Macías)
- fix: copiado y pegado page.tsx de develop para evitar errores en el merge (@Delfin)
- fix: admin error en panel de usuario (@DelfinSR)
- fix: todo arreglado (@Julen Redondo)
- fix: solucionar problemas ocn la subida de archivos (@Francisco Mateos Villarejo)
- fix: solucionar problemas de SonarCloud (@Francisco Mateos Villarejo)
- fix: revision del documento y correccion de faltas de ortografia (@Miguel Encina)
- fix: unsuscribe en boundary para el despliegue (@Benjamín Maureira Flores)
- fix: comentado el botón de subir vídeo en el perfil de fisio (@guaridpin)
- fix: correcciones menores de coherencia (@guaridpin)
- fix: mejorado estilo y quitados console.error para evitar que salten errores (@DelfinSR)
- fix: solucionado problema de que si no habia camara/microfono saltaba error (@DelfinSR)
- fix: Hacer los nuevos campos del perfil del fisio obligatorios (@Daniel Ruiz López)
- fix: for code scanning alert no. 68: Information exposure through an exception (@Miguel Encina)
- fix: añadir el Alert en el success (@Daniel Ruiz López)
- fix: bloquear formularios de dni y password en perfil de paciente y eliminado codigo que ya no se usa (@DelfinSR)
- fix: actualizado serializer de perfil de paciente para que no falle si no se envia el dni o la fecha de naciemiento porque son valores que no se pueden actualizar y añadida linea para que elimine account_status de los cambios a actualizar de paciente (@DelfinSR)
- fix: arreglado formato de fechas al crear una cita en frontend (@MiguelEncina)
- fix: calendario mejorado (@Daniel Ruiz López)
- fix: cambio menor en la redirección del fin de llamada (@DanTorBar)
- fix: error al traducir una palabra (@Francisco Mateos Villarejo)
- fix: fisioterapeuta no puede valorar citas (fin de videollamada) (@DanTorBar)
- fix: fisioterapeuta no puede valorar citas (@DanTorBar)
- fix: solucionado error en merge (@DelfinSR)
- fix: Botón Buscar fisioterapeutas (@Daniel Ruiz López)
- fix: problema con los ejercicios (@guaridpin)
- fix: arreglo para cargar correctamente las valoraciones (@guaridpin)
- fix: error al cargar biblioteca de ejercicios en perfil del fisio (@guaridpin)
- fix: Correcciñon de valoración global del fisio si no tiene valoraciones (@DanTorBar)
- fix: borrado print para que lo salgan msgs (@DelfinSR)
- fix: comprobacion de dni unico arreglado (@DelfinSR)
- fix: eliminación de componente sobrante (@DanTorBar)
- fix: arreglado sessionStorage de draftAppointment (@DelfinSR)
- fix: corrección de errores menores (@MiguelEncina)
- fix: borrado tsconfig que no se usa (@DelfinSR)
- fix: arreglado problema de distinto formatos de horas en citas entre servidor y frontend (@DelfinSR)
- fix: rating app renombrado para evitar conflictos (@DanTorBar)
### 📖 Documentation (91)
- docs: actualización CHANGELOG para 6.0.0 (@github-actions[bot])
- docs: asignación revisor (@albcarsic)
- docs: corrección errores (@albcarsic)
- docs: actualización CHANGELOG para 5.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 4.0.0 (@github-actions[bot])
- docs: primera versión doc feedback 25/04 (@albcarsic)
- docs: cambiando revisores (@Antonio Macías)
- docs: actualización CHANGELOG para 16.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 15.0.0 (@github-actions[bot])
- Merge pull request #536 from Proyecto-ISPP/docs/doc-feedback-11-04 (@Delfin)
- docs: actualización CHANGELOG para 14.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 13.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 12.0.0 (@github-actions[bot])
- docs: mid_term_retrospective (@DanielAlors)
- docs: actualización CHANGELOG para 11.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 10.0.0 (@github-actions[bot])
- Merge pull request #545 from Proyecto-ISPP/docs/lecciones-aprendidas-s1 (@Antonio Macías)
- docs: añadido link a excel a LessonsLearnedS1.md (@Delfin)
- docs: comentarios aplicados a LessonsLearnedS1.md (@Delfin)
- docs: creado LessonsLearnedS1.md (@Delfin)
- docs: version completa sprint planning ppl (@Antonio Macías)
- docs: Primera versión feedback 11-04 (@albcarsic)
- docs: actualización CHANGELOG para 9.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 8.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 7.0.0 (@github-actions[bot])
- docs: primera version sprint planning #PPL (@Antonio Macías)
- docs: actualización CHANGELOG para 6.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 5.0.0 (@github-actions[bot])
- docs: primera versión de análisis de costes de marketing y SEO (@guaridpin)
- docs: actualización CHANGELOG para 4.0.0 (@github-actions[bot])
- docs: primera version plan marketing (@Antonio Macías)
- docs: actualización CHANGELOG para 33.0.0 (@github-actions[bot])
- docs: demo sprint 3 (@Antonio Macías)
- docs: actualización CHANGELOG para 32.0.0 (@github-actions[bot])
- Merge pull request #505 from Proyecto-ISPP/docs/revision (@Guadalupe Ridruejo Pineda)
- docs: actualización CHANGELOG para 31.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 30.0.0 (@github-actions[bot])
- docs: creado documento revision (@DelfinSR)
- docs: actualización CHANGELOG para 29.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 28.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 27.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 26.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 25.0.0 (@github-actions[bot])
- docs: reporte BCC listo para entrega s3 (@Antonio Macías)
- docs: actualización CHANGELOG para 24.0.0 (@github-actions[bot])
- docs: reporte ia y tiempo listos para entrega s3 (@Antonio Macías)
- docs: Finalización puntos 4 y 5 (@albcarsic)
- docs: actualización CHANGELOG para 23.0.0 (@github-actions[bot])
- docs: presentacion y up listos para entrega s3 (@Antonio Macías)
- docs: actualización CHANGELOG para 22.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 21.0.0 (@github-actions[bot])
- docs: Informe de calidad s3 (@Francisco Mateos Villarejo)
- docs: actualización CHANGELOG para 20.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 19.0.0 (@github-actions[bot])
- Merge pull request #498 from Proyecto-ISPP/docs/time-report-sprint-3 (@Guadalupe Ridruejo Pineda)
- docs: actualización CHANGELOG para 18.0.0 (@github-actions[bot])
- docs: update de los sprint retrospectives (@Benjamín Maureira Flores)
- docs: actualización CHANGELOG para 17.0.0 (@github-actions[bot])
- docs: evaluación de rendiemiento #S3 (@Francisco Mateos Villarejo)
- docs: actualización CHANGELOG para 16.0.0 (@github-actions[bot])
- docs: retrospectiva del sprint 3 (@MiguelEncina)
- docs: actualización CHANGELOG para 15.0.0 (@github-actions[bot])
- docs: sprint retrospective grupal (@Benjamín Maureira Flores)
- docs: actualización CHANGELOG para 14.0.0 (@github-actions[bot])
- Merge pull request #500 from Proyecto-ISPP/docs/sprint-retrospective-grupo-3 (@Benjamín Maureira Flores)
- docs: Retrospectiva S3 Grupo 1 (@albcarsic)
- docs: Time report sprint 3 (@albcarsic)
- docs: Primera versión KB Report S3 (@albcarsic)
- docs: actualización CHANGELOG para 13.0.0 (@github-actions[bot])
- docs: adecuacion primeros documentos para el s3 (@Antonio Macías)
- docs: actualización CHANGELOG para 12.0.0 (@github-actions[bot])
- Merge pull request #497 from Proyecto-ISPP/feat/doc-ia-report-Sprint-3 (@Daniel Ruiz López)
- docs: Reporte de ia del Sprint 3 (@DaniFdezCab)
- docs: actualización CHANGELOG para 11.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 10.0.0 (@github-actions[bot])
- Merge pull request #492 from Proyecto-ISPP/docs/plan-de-pruebas (@Delfin)
- docs: añadido link a carpeta reportes seguridad a plan de pruebas (@Delfin)
- docs: actualización CHANGELOG para 9.0.0 (@github-actions[bot])
- docs: Revisión y corrección (@albcarsic)
- docs: creado plan de pruebas (@Delfin)
- docs: añadido apartado sobre politica de seguridad a reporte de seguridad de s3 (@DelfinSR)
- docs: añadido el security report s3 (@DelfinSR)
- docs: se ha cambiado el nombre a security_report por security_report-S2 (@DelfinSR)
- docs: actualización CHANGELOG para 8.0.0 (@github-actions[bot])
- docs: actualizado link form UP ISPP sprint3_comunicaciones_up (@Antonio Macías)
- docs: actualización CHANGELOG para 7.0.0 (@github-actions[bot])
- docs: actualizacion correo UP ISPP sprint3_comunicaciones_up (@Antonio Macías)
- docs: actualización CHANGELOG para 6.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 5.0.0 (@github-actions[bot])
- docs: actualización CHANGELOG para 4.0.0 (@github-actions[bot])
- docs: Documento de costes s3 (@Letee2)
### 🌀 Other Changes (99)
- Merge pull request #568 from Proyecto-ISPP/docs/doc-feedback-25-04 (@DanielAlors)
- chrome 135 baby (@Ramón Gavira)
- doc: version final del documento de planificación de campaña de marketing (@guaridpin)
- Merge pull request #553 from Proyecto-ISPP/doc/planning-wpl (@Antonio Macías)
- doc: planificacion previa wpl. correcciones ortigráficas (@Antonio Macías)
- doc: reajuste planificación #PPL (@Antonio Macías)
- Merge branch 'main' of github.com:Proyecto-ISPP/FISIOFIND (@guaridpin)
- doc: modificación del documento de marketing para añadir objetivos y anuncios de Spotify (@guaridpin)
- Merge pull request #506 from Proyecto-ISPP/develop (@Antonio Macías)
- Merge pull request #548 from Proyecto-ISPP/feat/accesibilidad (@Antonio Macías)
- style: añadir icono de subtitulos (@Ramón Gavira)
- Merge pull request #544 from Proyecto-ISPP/docs/sprint-planning-ppl (@Delfin)
- Update LessonsLearnedS1.md (@Guadalupe Ridruejo Pineda)
- Merge pull request #546 from Proyecto-ISPP/feat/reconocimiento-voz (@Benjamín Maureira Flores)
- style: arreglando logo y titulo de varios documentos (@Antonio Macías)
- Potential fix for code scanning alert no. 74: DOM text reinterpreted as HTML (@Antonio Macías)
- Merge branch 'feat/ver-documentos' of https://github.com/Proyecto-ISPP/FISIOFIND into feat/ver-documentos (@Antonio Macías)
- style: new patient treatment pages (@Antonio Macías)
- style: improoved style for file and video page (@Antonio Macías)
- Potential fix for code scanning alert no. 71: DOM text reinterpreted as HTML (@Antonio Macías)
- Merge remote-tracking branch 'origin/develop' into feat/ver-documentos (@Antonio Macías)
- Merge branch 'main' of github.com:Proyecto-ISPP/FISIOFIND (@guaridpin)
- doc: modificado análisis de costes de marketing (@guaridpin)
- Merge pull request #516 from Proyecto-ISPP/main (@Delfin)
- Merge pull request #504 from Proyecto-ISPP/hotfix/perfil-paciente (@Guadalupe Ridruejo Pineda)
- Merge pull request #502 from Proyecto-ISPP/develop (@Benjamín Maureira Flores)
- Merge pull request #501 from Proyecto-ISPP/docs/knowledge-base-report-s3 (@Antonio Macías)
- Merge remote-tracking branch 'origin/main' into docs/knowledge-base-report-s3 (@Antonio Macías)
- Merge pull request #499 from Proyecto-ISPP/doc/reporte-up-sprint-3 (@Antonio Macías)
- Update docs/03_reports/pilot_user_reports/sprint3_pilot_user_report.md (@Antonio Macías)
- Update docs/03_reports/pilot_user_reports/sprint3_pilot_user_report.md (@Antonio Macías)
- Merge branch 'main' of https://github.com/Proyecto-ISPP/FISIOFIND (@Francisco Mateos Villarejo)
- Merge branch 'main' of https://github.com/Proyecto-ISPP/FISIOFIND (@Francisco Mateos Villarejo)
- Merge branch 'main' of https://github.com/Proyecto-ISPP/FISIOFIND (@MiguelEncina)
- doc: se ha finalizado el informe de usuarios piloto correspondiente al s3 (@guaridpin)
- Merge pull request #494 from Proyecto-ISPP/doc/sprint-retrospectiveg2-s3 (@Antonio Macías)
- Merge branch 'main' into develop (@MiguelEncina)
- Merge pull request #496 from Proyecto-ISPP/hotfix/error-admin (@Miguel Encina)
- Merge pull request #495 from Proyecto-ISPP/feat/valoraciones-tests (@Miguel Encina)
- test: suite de tests de appointment_ratings (alto coverage) (@DanTorBar)
- doc: sprint retrospective (@Julen Redondo)
- Merge pull request #491 from Proyecto-ISPP/docs/reporte-seguridad-s3 (@albcarsic)
- doc: añadida justificación del aplazamiento de la prueba de UP externos (@guaridpin)
- Merge pull request #487 from Proyecto-ISPP/develop (@Guadalupe Ridruejo Pineda)
- Merge pull request #485 from Proyecto-ISPP/feat/pagos-y-facturas (@Guadalupe Ridruejo Pineda)
- Merge branch 'develop' into feat/pagos-y-facturas (@Guadalupe Ridruejo Pineda)
- Merge remote-tracking branch 'origin/develop' into feat/ver-documentos (@Antonio Macías)
- Merge pull request #489 from Proyecto-ISPP/fix/solucionar-videollamadas-error-si-no-dispositivos (@Guadalupe Ridruejo Pineda)
- Merge pull request #452 from Proyecto-ISPP/test/test-unitarios (@Daniel Ruiz López)
- Merge pull request #483 from Proyecto-ISPP/feat/usuario-actualizar-contraseña (@Daniel Ruiz López)
- Merge branch 'develop' into feat/usuario-actualizar-contraseña (@Delfin)
- Merge pull request #481 from Proyecto-ISPP/feat/solucionar-problemas-seguridad (@Daniel Fernández)
- Merge branch 'develop' of https://github.com/Proyecto-ISPP/ISPP into feat/solucionar-problemas-seguridad (@DaniFdezCab)
- Merge branch 'develop' into test/test-unitarios (@DelfinSR)
- Merge branch 'develop' into feat/usuario-actualizar-contraseña (@DelfinSR)
- Merge pull request #486 from Proyecto-ISPP/feat/refactorizacion-frontend (@Daniel Ruiz López)
- Merge branch 'develop' into feat/refactorizacion-frontend (@Daniel Ruiz López)
- refactor: Mejora de estilos del generador del cuestionario y eliminación de código inutilizado (@Rafa)
- refactor: mejora de estilos en la página del cuestionario del fisio (@Rafa)
- refactor: mejora de estilos de los ejercicios del fisio y de las sesiones (@Rafa)
- Merge pull request #484 from Proyecto-ISPP/feat/verificacion-fisio (@Daniel Vela Camacho)
- Merge pull request #473 from Proyecto-ISPP/feat/valoraciones (@Miguel Encina)
- Merge branch 'develop' of https://github.com/Proyecto-ISPP/ISPP into feat/pagos-y-facturas (@franciiscocg)
- Merge branch 'feat/refactorizacion-frontend' of https://github.com/Proyecto-ISPP/FISIOFIND into feat/refactorizacion-frontend (@Daniel Ruiz López)
- Merge remote-tracking branch 'origin/develop' into feat/valoraciones (@DanTorBar)
- test: tests de ratings (@DelfinSR)
- Merge branch 'develop' of https://github.com/Proyecto-ISPP/FISIOFIND into feat/verificacion-fisio (@Francisco Mateos Villarejo)
- Merge branch 'develop' into test/test-unitarios (@DelfinSR)
- Merge branch 'feat/refactorizacion-frontend' of https://github.com/Proyecto-ISPP/FISIOFIND into feat/refactorizacion-frontend (@Rafa)
- refactor: mejorar estilos de la página de ejercicios del fisio (@Rafa)
- Merge branch 'develop' into feat/usuario-actualizar-contraseña (@DelfinSR)
- Merge pull request #469 from Proyecto-ISPP/hotfix/cancelar-rececpcion-notificaciones (@Miguel Encina)
- tests: tests url cambiar contraseña (@DelfinSR)
- Merge branch 'feat/refactorizacion-frontend' of https://github.com/Proyecto-ISPP/FISIOFIND into feat/refactorizacion-frontend (@Daniel Ruiz López)
- refactor: Mejorar el estilo de la interfaz de usuario y eliminar las variables de estado no utilizadas. (@Rafa)
- Merge branch 'develop' of https://github.com/Proyecto-ISPP/FISIOFIND into feat/verificacion-fisio (@Francisco Mateos Villarejo)
- Merge branch 'feat/valoraciones' of github.com:Proyecto-ISPP/FISIOFIND into feat/valoraciones (@guaridpin)
- Merge branch 'feat/valoraciones' of https://github.com/Proyecto-ISPP/FISIOFIND into feat/valoraciones (@DanTorBar)
- refactor: cambiar etiquetas de navegación para usar Links (@Rafa)
- Merge branch 'develop' into hotfix/cancelar-rececpcion-notificaciones (@danvelcam@alum.us.es)
- Merge branch 'develop' into feat/valoraciones (@guaridpin)
- Update backend/fisio_find/appointment_rating/views.py (@Guadalupe Ridruejo Pineda)
- Merge remote-tracking branch 'origin/develop' into feat/valoraciones (@DanTorBar)
- Merge branch 'develop' into feat/solucionar-problemas-seguridad (@DelfinSR)
- test: actualizados y revisados tests de user (@DelfinSR)
- test: tests de videocall creados (@DelfinSR)
- Merge branch 'develop' into test/test-unitarios (@DelfinSR)
- Merge branch 'feat/front-subida-archivos', remote-tracking branch 'origin' into feat/ver-documentos (@Julen Redondo)
- Merge branch 'develop' into test/test-unitarios (@DelfinSR)
- chore: modificado nombre fichero para matchear el componente (@danvelcam)
- chore: eliminado un print usado para debuggear (@danvelcam)
- Merge branch 'develop' into hotfix/cancelar-rececpcion-notificaciones (@danvelcam)
- Merge remote-tracking branch 'origin/develop' into feat/valoraciones (@DanTorBar)
- Merge branch 'develop' into test/test-unitarios (@DelfinSR)
- test: tests busqueda avanzada creados (@DelfinSR)
- Merge remote-tracking branch 'origin/develop' into feat/valoraciones (@DanTorBar)
- Merge branch 'test/test-unitarios' of https://github.com/Proyecto-ISPP/FISIOFIND into test/test-unitarios (@MiguelEncina)
- Merge branch 'develop' into test/test-unitarios (@DelfinSR)
- Merge remote-tracking branch 'origin/main' into feat/valoraciones (@DanTorBar)
### 🔗 Pull Requests
- [#568](https://github.com/Proyecto-ISPP/FISIOFIND/pull/568) - Merge pull request #568 from Proyecto-ISPP/docs/doc-feedback-25-04
- [#553](https://github.com/Proyecto-ISPP/FISIOFIND/pull/553) - Merge pull request #553 from Proyecto-ISPP/doc/planning-wpl
- [#536](https://github.com/Proyecto-ISPP/FISIOFIND/pull/536) - Merge pull request #536 from Proyecto-ISPP/docs/doc-feedback-11-04
- [#506](https://github.com/Proyecto-ISPP/FISIOFIND/pull/506) - Merge pull request #506 from Proyecto-ISPP/develop
- [#548](https://github.com/Proyecto-ISPP/FISIOFIND/pull/548) - Merge pull request #548 from Proyecto-ISPP/feat/accesibilidad
- [#544](https://github.com/Proyecto-ISPP/FISIOFIND/pull/544) - Merge pull request #544 from Proyecto-ISPP/docs/sprint-planning-ppl
- [#545](https://github.com/Proyecto-ISPP/FISIOFIND/pull/545) - Merge pull request #545 from Proyecto-ISPP/docs/lecciones-aprendidas-s1
- [#546](https://github.com/Proyecto-ISPP/FISIOFIND/pull/546) - Merge pull request #546 from Proyecto-ISPP/feat/reconocimiento-voz
- [#475](https://github.com/Proyecto-ISPP/FISIOFIND/pull/475) - Merge pull request #475 from Proyecto-ISPP/feat/ver-documentos
- [#516](https://github.com/Proyecto-ISPP/FISIOFIND/pull/516) - Merge pull request #516 from Proyecto-ISPP/main
- [#505](https://github.com/Proyecto-ISPP/FISIOFIND/pull/505) - Merge pull request #505 from Proyecto-ISPP/docs/revision
- [#504](https://github.com/Proyecto-ISPP/FISIOFIND/pull/504) - Merge pull request #504 from Proyecto-ISPP/hotfix/perfil-paciente
- [#503](https://github.com/Proyecto-ISPP/FISIOFIND/pull/503) - Merge pull request #503 from Proyecto-ISPP/hotfix/perfil-paciente
- [#502](https://github.com/Proyecto-ISPP/FISIOFIND/pull/502) - Merge pull request #502 from Proyecto-ISPP/develop
- [#501](https://github.com/Proyecto-ISPP/FISIOFIND/pull/501) - Merge pull request #501 from Proyecto-ISPP/docs/knowledge-base-report-s3
- [#499](https://github.com/Proyecto-ISPP/FISIOFIND/pull/499) - Merge pull request #499 from Proyecto-ISPP/doc/reporte-up-sprint-3
- [#493](https://github.com/Proyecto-ISPP/FISIOFIND/pull/493) - Merge pull request #493 from Proyecto-ISPP/feat/update-readme
- [#498](https://github.com/Proyecto-ISPP/FISIOFIND/pull/498) - Merge pull request #498 from Proyecto-ISPP/docs/time-report-sprint-3
- [#500](https://github.com/Proyecto-ISPP/FISIOFIND/pull/500) - Merge pull request #500 from Proyecto-ISPP/docs/sprint-retrospective-grupo-3
- [#494](https://github.com/Proyecto-ISPP/FISIOFIND/pull/494) - Merge pull request #494 from Proyecto-ISPP/doc/sprint-retrospectiveg2-s3
- [#497](https://github.com/Proyecto-ISPP/FISIOFIND/pull/497) - Merge pull request #497 from Proyecto-ISPP/feat/doc-ia-report-Sprint-3
- [#496](https://github.com/Proyecto-ISPP/FISIOFIND/pull/496) - Merge pull request #496 from Proyecto-ISPP/hotfix/error-admin
- [#495](https://github.com/Proyecto-ISPP/FISIOFIND/pull/495) - Merge pull request #495 from Proyecto-ISPP/feat/valoraciones-tests
- [#492](https://github.com/Proyecto-ISPP/FISIOFIND/pull/492) - Merge pull request #492 from Proyecto-ISPP/docs/plan-de-pruebas
- [#491](https://github.com/Proyecto-ISPP/FISIOFIND/pull/491) - Merge pull request #491 from Proyecto-ISPP/docs/reporte-seguridad-s3
- [#487](https://github.com/Proyecto-ISPP/FISIOFIND/pull/487) - Merge pull request #487 from Proyecto-ISPP/develop
- [#485](https://github.com/Proyecto-ISPP/FISIOFIND/pull/485) - Merge pull request #485 from Proyecto-ISPP/feat/pagos-y-facturas
- [#489](https://github.com/Proyecto-ISPP/FISIOFIND/pull/489) - Merge pull request #489 from Proyecto-ISPP/fix/solucionar-videollamadas-error-si-no-dispositivos
- [#452](https://github.com/Proyecto-ISPP/FISIOFIND/pull/452) - Merge pull request #452 from Proyecto-ISPP/test/test-unitarios
- [#488](https://github.com/Proyecto-ISPP/FISIOFIND/pull/488) - Merge pull request #488 from Proyecto-ISPP/feat/cambiar-claves-digitalocean
- [#483](https://github.com/Proyecto-ISPP/FISIOFIND/pull/483) - Merge pull request #483 from Proyecto-ISPP/feat/usuario-actualizar-contraseña
- [#481](https://github.com/Proyecto-ISPP/FISIOFIND/pull/481) - Merge pull request #481 from Proyecto-ISPP/feat/solucionar-problemas-seguridad
- [#486](https://github.com/Proyecto-ISPP/FISIOFIND/pull/486) - Merge pull request #486 from Proyecto-ISPP/feat/refactorizacion-frontend
- [#484](https://github.com/Proyecto-ISPP/FISIOFIND/pull/484) - Merge pull request #484 from Proyecto-ISPP/feat/verificacion-fisio
- [#473](https://github.com/Proyecto-ISPP/FISIOFIND/pull/473) - Merge pull request #473 from Proyecto-ISPP/feat/valoraciones
